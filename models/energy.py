...
# declarative class is the parent class for all db models 

from sqlalchemy.orm import sessionmaker 
# session_maker creates database sessions. 
# A session is similar to connection used for INSERT,UPDATE,DELETE,SELECT 
import os 

# Instead of working with raw SQL we will make use of Python classes and objects 

Base = declarative_base()

# EnergyUsageTable represents CREATE TABLE energy_usage
...
class DatabaseManager:
    ...
    def add_usage_record(self,
        ...
        try:
            record = EnergyUsage(timestamp=timestamp,
                                 consumption_kwh=consumption_kwh,
                                 device_type=device_type,
                                 device_name=device_name,
                                 cost_usd=cost_usd)
            
            session.add(record) # inserts row
    ...
    def add_generation_record(self,
                              timestamp: datetime,
        ...
        session = self.get_session()
        try:
            record = SolarGeneration(timestamp=timestamp,
    ...
    def get_usage_by_date_range(self,
                                start_date: datetime,
                                end_date: datetime):
        ...
        session = self.get_session()
        try:
            return session.query(EnergyUsage).filter(EnergyUsage.timestamp >= start_date,
                                                     EnergyUsage.timestamp <= end_date
                                                     ).order_by(EnergyUsage.timestamp).all()
        finally:
            session.close()

    def get_generation_by_date_range(self,
                                     start_date: datetime,
                                     end_date: datetime):
        """Get solar generation records within date range"""
        session = self.get_session()
        try:
            return session.query(SolarGeneration).filter(SolarGeneration.timestamp >= start_date,
